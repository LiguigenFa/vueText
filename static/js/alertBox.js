...
var alertBox = function alertBox() {
	...
	var GenerateCss = function() {
		...
		$("#tipBox-ico").css({
			...
			height: '17px',
			marginTop: '15px',
			background: 'url(../../static/assets/img3/close.png) no-repeat',
			cursor: 'pointer'
		});
		$("#tipBox-content").css({
			fontSize: '14px',
		...
		$("#tipBox-btn-ok,#tipBox-btn-no").css({ width: '102px', height: '40px', fontSize: '14px', color: '#fff', border: 'none', background: '#fe5959', borderRadius: '3px' });
		$("#tipBox-btn-no").css({ color: '#989898', backgroundColor: '#fff', border: '1px solid #dbdbdb', marginRight: '15px' });
		//关闭按钮事件
	...
	$(".logout").click(function() {
		$.MsgBox.Confirm("温馨提示", "确定退出？", "icon-exit", function() { console.log("点击确定按钮的回调函数") });
	})
}
